...
verificaProg c1 = b1 where 
                (s1,b1) = verificaCmd [] c1

avaliaProg :: Cmd -> Maybe Int
avaliaProg c1 = if verificaProg c1 == False then
                    Nothing
                else 
                    Just (cosulta mem' "ret") where 
                        mem' = avaliaCmd [] c1 
